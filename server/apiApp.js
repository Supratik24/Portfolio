...
const profileSchema = z
  .object({
    ...
    about: z.object({
      paragraphs: z.array(z.string().min(10).max(400)).min(1).max(6),
      values: z
        .array(
          z.object({
            title: z.string().min(2).max(40),
            body: z.string().min(10).max(220),
          }),
        )
        .min(1)
        .max(8),
      toolset: z.array(z.string().min(1).max(50)).min(1).max(20),
    }),
  })
  .strict();
...
async function getSite() {
  ...
  if (doc) return doc;
  const created = await Site.create({ key: "main", profile: null, skills: {}, projects: [], updatedAt: new Date() });
...
export function createApiApp() {
  ...
  app.get("/api/site/profile", async (req, res) => {
    ...
    if (!dbReady) return res.status(503).json({ error: "db_unavailable" });
    const s = await getSite();
    res.json({ profile: s.profile ?? null });
  });
  ...
  app.get("/api/admin/site", requireAdmin, async (req, res) => {
    ...
    if (!dbReady) return res.status(503).json({ error: "db_unavailable" });
    const s = await getSite();
    res.json({ profile: s.profile ?? null, skills: s.skills ?? {}, projects: s.projects ?? [] });
  });
